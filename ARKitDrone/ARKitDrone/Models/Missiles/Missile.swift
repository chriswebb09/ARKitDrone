...
class Missile {
    ...
    private static let registryQueue = DispatchQueue(label: "missile.registry", attributes: .concurrent)
    ...
    func fireToPosition(_ targetPosition: SIMD3<Float>) {
        ...
        entity.transform.rotation = simd_quatf(from: SIMD3<Float>(0, 0, 1), to: direction)
        ...
        let moveAnimation = FromToByAnimation<Transform>(
            name: "missileFlight",
            from: Transform(rotation: entity.transform.rotation, translation: start),
            to: Transform(rotation: entity.transform.rotation, translation: targetPosition),
            duration: duration,
            timing: .linear,
