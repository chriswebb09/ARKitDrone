...
extension SCNGeometry {
    ...
    static func from(_ arGeometry: ARMeshGeometry, ofType type: ARMeshClassification) -> SCNGeometry? {
        ...
        let bounds = vertices.reduce((SCNVector3Zero, SCNVector3Zero)) { (bbox, vertex) -> (SCNVector3, SCNVector3) in
            return (
                SCNVector3(x: .minimum(bbox.0.x, vertex.x), y:  .minimum(bbox.0.y, vertex.y), z: .minimum(bbox.0.z, vertex.z)),
                SCNVector3(x: .maximum(bbox.1.x, vertex.x), y: .maximum(bbox.1.y, vertex.y), z: .maximum(bbox.1.z, vertex.z))
            )
        }
